...
def run_backtest(ver_dir, ver_name, use_recent_100=True):
    ...
    def load_mod(fp):
        spec = importlib.util.spec_from_file_location(f'm_{ver_name}_{os.path.basename(fp).replace(".","_")}', fp)
    ...
    return rate, total, wins, detail

print("=" * 60)
print("  V42 vs V44 100天胜率验证")
print("=" * 60)

for ver_name, ver_file in [('V42', 'V42'), ('V44', 'V44')]:
    ver_dir = os.path.join(BASE, 'release', ver_file)
    print(f"\n▶ 运行 {ver_name} 100天回测...")
    rate, total, wins, detail = run_backtest(ver_dir, ver_name, use_recent_100=True)
    print(f"   {detail}")
...
for ver_name, ver_file in [('V42', 'V42'), ('V44', 'V44')]:
    ver_dir = os.path.join(BASE, 'release', ver_file)
    print(f"\n▶ 运行 {ver_name} 200天回测...")
    rate, total, wins, detail = run_backtest(ver_dir, ver_name, use_recent_100=False)
    print(f"   {detail}")
...
print(f"""
  ...
  注意：数据库是用"2:50尾盘回测"数据跑的
  我的回测是用"收盘价"数据跑的
  所以我的胜率可能会低一些（2:50 vs 收盘有偏差）
