#!/usr/bin/env python3
"""2026-01-09 vs 2026-01-12 Top5对比"""
import pickle, os
...
for target_dt in ['2026-01-09','2026-01-12']:
    cs=[e for e in dc.get(target_dt,[]) if ok(e)]
    for e in cs: e['s2']=round(sc(e),2)
    cs.sort(key=lambda e:e['s2'], reverse=True)
    print("\n"+"="*140)
    print("  %s Top5（共%d只）"%(target_dt, len(cs)))
    print("="*140)
    print("#  名称      代码              买入价  涨跌   ATR  位  上影  DIF    DEA   差    K    D    J   评分  次日")
    print("-"*110)
    for i,e in enumerate(cs[:5],1):
        n2=nm.get(e['code'],'?')
        nxt="%.1f%%"%e['n'] if e['n'] is not None else "N/A"
        mg=e.get('macd_gap',0)
        print("%d  %-8s %-16s %7.2f %+.1f%% %4.1f%% %3.0f%% %4.1f%% %5.2f %5.2f %4.2f %3.0f %3.0f %3.0f %5.1f %s"%(i,n2,e['code'],e['close'],e['p'],e['a'],e.get('cl',0),e.get('s',0),e.get('dif_val',0)or 0,e.get('dea_val',0)or 0,mg,e.get('k_val',0)or 0,e.get('d_val',0)or 0,e.get('j_val',0)or 0,e['s2'],nxt))
    print("-"*110)
    c1=cs[0]
    n2=nm.get(c1['code'],'?')
    res="达标" if c1['n']>=2.5 else "失败"
    print("  冠军: %s(%s) 评分%.1f → 次日%s = %s"% (n2,c1['code'],c1['s2'],"%.1f%%"%c1['n'] if c1['n'] else "N/A",res))
