#!/usr/bin/env python3
"""V22 长期稳定版 每日选股报告 — 实时API + V10同款淡色模板
胜率：30天=73%/50天=68%/100天=68%（历史close数据）
      30天=66.7%/50天=62%/100天=64%（最新close数据）
特点：WR双层评分+6重衰竭过滤，各窗口胜率最稳定（几乎不衰减）
"""
...
V22_DIR = os.path.join(SCRIPTS_DIR, 'release', 'V22')
CACHE_DIR = os.path.expanduser('~/AppData/Local/hermes/hermes-agent/cache')
sys.path.insert(0, V22_DIR); sys.path.insert(0, SCRIPTS_DIR)
...
for n in ['真实涨日','虚涨日','跌日','横盘']:
    fp = os.path.join(V22_DIR, '评分策略', f'分而治之_V10_{n}_评分策略.py')
...
def backtest_30d():
    ...
    recent = all_dates[-30:]
    extended = all_dates[all_dates.index(recent[0])-6:all_dates.index(recent[-1])+1]
    ...
    for dt in recent:
        ...
        top3_results.append({
            ...
            'p':champ.get('p',0) or 0,'nh':nh,'score':scored[0][0],
    ...
    return wi, ta, mk_s, daily_rows, top3_results, recent[-1]

# ===== 生成HTML =====
def build_html(top10_result, bt_result, today_str, data_date_str, buy_date_str, mk_cn, level):
    """V10同款淡色模板"""
    wi, ta, mk_s, daily_rows, top3_results, last_cache_date = bt_result
    now = datetime.now()
...
<tr style="background:#f5f6fa;font-weight:bold"><td>🎯 Top3任一达标</td><td style="color:#e74c3c;font-size:16px">{top3_any*100//n}%</td><td>{top3_any}/{n}</td><td>三只票买任意一只，{n}天中{top3_any}天有票达标</td></tr>'''
    ...
    for r in reversed(top3_results):
        ...
        daily_table += f'''<tr><td>{r['dt'][5:]}</td><td>{r['mk']}</td><td style='font-weight:bold'>{r['c_name']}</td><td>{r['c_code']}</td><td style='color:{pct_c};font-weight:bold'>{r['p']:+.1f}%</td><td style='color:{nh_c};font-weight:bold'>{r['nh']:+.1f}%</td><td style='font-weight:bold'>{r['score']:.0f}</td><td>{r['cl']:.0f}</td><td>{r['vr']:.2f}</td><td>{r['hsl']:.1f}</td><td>{r['wr']:.0f}</td><td>{r['dif']:.2f}</td><td>{r['result']}</td></tr>'''
...
<title>V22长期稳定版每日选股报告 {today_str}</title>
<style>
...
2. K线API → MACD/KDJ/WR/CL等技术指标
3. 行情分类（均价涨幅>0.5% → 真实涨日/虚涨日，<-0.5% → 跌日，其余→横盘）
4. L→L5分级筛选（硬门槛逐级放宽至≥10只）
5. V22评分（WR双层评分+6重衰竭过滤）
6. 淡色HTML输出 → 存档 + 邮件
</pre>
</div>
...
<div class="card">
<h3>📋 V22 vs V10 对比</h3>
<table><thead><tr><th>指标</th><th>V22</th><th>V10</th></tr></thead><tbody>
<tr><td>最新30天胜率</td><td style="color:#e74c3c;font-weight:bold">66.7% (20/30)</td><td>86.2% (25/29)</td></tr>
<tr><td>全年胜率</td><td>68% (稳定窗口)</td><td>65.6%</td></tr>
<tr><td>核心改进</td><td>WR双层评分+6重衰竭过滤</td><td>老司机数据驱动</td></tr>
</tbody></table>
</div>

<div class="footer">
V22 长期稳定版 | 30天66.7% | 各窗口最稳定
...
if __name__ == '__main__':
    ...
    print(f'🚀 V22 长期稳定版 每日选股报告 | {today_str}', flush=True)
    ...
    if top10_raw:
        ...
        try:
            ...
            save_realtime_to_datacache(today_str, stocks, indicators, 'tencent:1450-v22')
    ...
    if top10 and bt:
        ...
        fp = os.path.join(arch, f'{today_str}_V22_报告.html')
        ...
        try:
            ...
            subj = f'V22长期稳定版 {today_str} - {top10[1]} - 冠军{top10[0][0][1]["name"]}[{top10[0][0][1]["price"]:.2f}]'
        ...
        # 写入数据库日志
        try:
            log_selection_to_db('V22', today_str, top10[1], top10[2], len(top10[0]), top10[0])
        except Exception as e:
        ...
        print(f'近30天: {bt[0]}/{bt[1]} = {bt[0]*100/bt[1]:.1f}%')
    else:
        print('❌ 选股失败')
