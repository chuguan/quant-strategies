...
def box_break(code, date):
    ...
    try:
        ...
        c = kdata[idx]['close']
        # 箱体突破或接近突破
        if c > h20: return 5  # 明确突破
        if c >= h20 * 0.98: return 3  # 接近突破
        return 0
    except: return 0
...
tests = {
    '涨x2+CL+价+MACD+5日': lambda c: c[3]*2 + c[0] + ps(c[7])*0.5 + ms(c[9],c[10])*0.5 + 10*c[11],
    '+箱体+5': lambda c: c[3]*2 + c[0] + ps(c[7])*0.5 + ms(c[9],c[10])*0.5 + 10*c[11] + c[12],
}
...
for label, sort_key in tests.items():
    ...
    print(f'{label:<25}: {wins}/{ndays}({wins*100/ndays:.1f}%)')
