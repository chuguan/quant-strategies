...
BONUS = [5, 3, 0, -2, -5]  # L0~L4级别加分

total=0; wins=0; by_month={}
used_expand=0; used_l2=0
...
for dt in dates:
    ...
    for li in range(len(LEVELS)):
        ...
        for s in stocks:
            ...
            seen.add(code)
            candidates.append((code,p,cl,vr,hsl,nm,li,nh,s))
            added=True
            
            if len(seen)>=target_n: break
        if li>0 and added: used_expand+=1
        if li>=2 and added: used_l2+=1
        if len(seen)>=target_n: break
    ...
    # 评分
    scored=[]
    for code,p,cl,vr,hsl,nm,li,nh,s in candidates:
...
print(f"\n{'='*60}", flush=True)
print(f"  v11完整版 (分级放宽+级别加分+行情切换)", flush=True)
...
print(f"需放宽:{used_expand}天 需L2以上:{used_l2}天", flush=True)
