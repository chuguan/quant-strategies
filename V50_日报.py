...
V50_DIR = os.path.join(SCRIPTS_DIR, 'release', 'V50')
...
# ===== 生成HTML =====
def build_html(top10_result, bt_result, today_str, data_date_str, buy_date_str, mk_cn, level, ver_id='V50'):
    """V10同款淡色模板"""
...
<title>V50每日选股报告 {today_str}</title>
<style>
...
<div style="font-size:22px;font-weight:bold;color:#b8860b;text-align:center;margin:10px 0;background:linear-gradient(135deg,#fff8e1,#ffecb3);padding:12px;border-radius:8px;border-bottom:3px solid #b8860b">
  V50 分而治之 · 每日选股报告 (30天{wi*100//ta}%版)
</div>
...
5. <b>V50评分</b> — V41横盘混合评分 + 冲顶惩罚 + HSL>18%一票否决防主力出货
...
<div class="card">
<h3>📋 V50 vs V13 对比</h3>
<table><thead><tr><th>指标</th><th>V50 🏆</th><th>V13</th></tr></thead><tbody>
...
<div class="footer">
V50 分而治之 | 30天 {wi}/{ta} = {wi*100//ta}% | 实战累计中
...
if __name__ == '__main__':
    ...
    if top10 and bt:
        ...
        try:
            ...
            subj = f'V50每日选股 {today_str} - {top10[1]} - 冠军{top10[0][0][1]["name"]}[{top10[0][0][1]["price"]:.2f}]'
        ...
        # 发邮件B（潘永俊）
        try:
            subj_b = f'V50每日选股 {today_str} - {top10[1]} - 冠军{top10[0][0][1]["name"]}[{top10[0][0][1]["price"]:.2f}]'
