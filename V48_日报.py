...
sys.path.insert(0, V42_DIR); sys.path.insert(0, SCRIPTS_DIR)
...
# ===== 生成HTML =====
def build_html(top10_result, bt_result, today_str, data_date_str, buy_date_str, mk_cn, level, ver_id='V42'):
    """V10同款淡色模板"""
...
<div style="font-size:22px;font-weight:bold;color:#b8860b;text-align:center;margin:10px 0;background:linear-gradient(135deg,#fff8e1,#ffecb3);padding:12px;border-radius:8px;border-bottom:3px solid #b8860b">
  V42 分而治之 · 每日选股报告 (30天{wi*100//ta}%版)
</div>
...
<tr><td>最强行情</td><td>虚涨日100% 🏆 | 横盘78% 🏆 | 跌日78% 🏆</td></tr>
<tr><td>短板行情</td><td>真实涨日 ~72%</td></tr>
<tr><td>选它理由</td><td>🏆 总榜冠军！所有行情均衡，HSL否决防主力出货</td></tr>
</tbody></table>
</div>
...
5. <b>V42评分</b> — V41横盘混合评分 + 冲顶惩罚 + HSL>18%一票否决防主力出货
...
<div class="card">
<h3>📋 V42 vs V13 对比</h3>
<table><thead><tr><th>指标</th><th>V42 🏆</th><th>V13</th></tr></thead><tbody>
<tr><td>最新30天胜率</td><td style="color:#e74c3c;font-weight:bold">{wi*100//ta}% ({wi}/{ta})</td><td style="color:#e74c3c;font-weight:bold">96.7% (29/30)</td></tr>
<tr><td>50天胜率</td><td><b>82%</b> 🏆 (回测)</td><td>68% (回测)</td></tr>
<tr><td>100天胜率</td><td><b>77%</b> 🏆 (回测)</td><td>61% (回测)</td></tr>
<tr><td>核心武器</td><td>横盘CL<88混合评分+冲顶惩罚+HSL否决</td><td>动量衰竭6规则+7天衰减扣分</td></tr>
<tr><td>最强行情</td><td>虚涨日100% 🏆 横盘78% 🏆 跌日78% 🏆</td><td>真实涨日~79%</td></tr>
<tr><td>短板行情</td><td>真实涨日~72%</td><td>横盘~56% 虚涨日~20%</td></tr>
</tbody></table>
</div>
...
if __name__ == '__main__':
    ...
    print(f'🚀 V48 每日选股报告 | {today_str}', flush=True)
    ...
    if top10 and bt:
        ...
        # 发邮件B（潘永俊）
        try:
            subj_b = f'V42每日选股 {today_str} - {top10[1]} - 冠军{top10[0][0][1]["name"]}[{top10[0][0][1]["price"]:.2f}]'
