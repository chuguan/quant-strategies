...
from _config import VERSION_DIR as DIR, STRATEGY_DIR
...
# 30天回测
dates = sorted(k for k in data.keys() if '2026-04-14' <= k <= '2026-06-03')
recent = dates[-30:]
wi = 0; ta = 0
top3win = {}
print(f'\n===== 1150 ({recent[0]}~{recent[-1]}) =====')
for dt in recent:
    ...
    nh = champ.get('n',0) or 0; nm2 = names.get(champ['code'],'?'); pv = champ.get('p',0)
    ta += 1
    if nh >= 2.5: wi += 1; sta = 'OK'
    else:
        fe = precomputed.get((champ['code'], dt), {})
        sta = f'FAIL(sl5={fe.get("slope5",0):.1f} t4s={fe.get("t4_shadow",0):.0f})'
    print(f'{dt} {mk_cn:>5} {nm2:>10} p={pv:.1f}% sc={champ_sc:.0f} nh={nh:+.1f}% {sta}')
