...
def score(stock):
    ...
    # ═══ V49：跌日高换手+高位扣分 (HSL>12或Pos>75=出货风险) ═══
    if h > 12 or po > 75:
        s -= 8
    
    # ═══ V50：5日斜率向上加分 (ma5_slope>5说明短期趋势强) ═══
    ms5 = stock.get('ma5_slope', 0) or 0
    if ms5 > 5:
        s += 10
    
    # ═══ 缩量涨扣分（华通线缆案例：VR=0.88缩量涨→次日崩盘）═══
    if v < 0.9: s -= 8
    # ═══ 高位超买+死叉扣分（安孚科技案例：CL=96.4,WR=4,DIF=-5.3→冲高回落）═══
    # ═══ DIF虚高扣分（海思科/瑞芯微案例：DIF>2但p<3=见顶）═══
    if d > 2.0 and p < 3.0: s -= 12
    # ═══ 前日涨停追涨扣分（盛新锂能/百傲化学案例：前日涨停次日续涨=高风险）═══
