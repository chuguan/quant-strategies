...
def score(stock):
    ...
    if pp > 5: bs -= 3
    if po > 70: bs -= 8
    if c > 85 and w < 15 and d < 0: bs -= 12  # 高位超买(CL>85+WR<15)+死叉(DIF<0)
    if v < 0.9: bs -= 8  # 缩量涨（VR<0.9）是虚涨，有崩盘风险
    # ═══ DIF虚高扣分 ═══
    if d > 2.0 and pp < 3.0: bs -= 12  # DIF虚高但涨幅不够
    # ═══ 前日涨停追涨扣分 ═══
    if stock.get("d1",0) > 8: bs -= 8  # 前日涨停追涨风险扣分
