#!/usr/bin/env python3
"""CG18 模拟盘每日买入 — 13:10执行，从DB读取最新冠军买入"""
...
try:
    ...
    cur = conn.execute(
        ...
        "WHERE version='CG18' AND rank=1 ORDER BY run_time DESC LIMIT 1",
    )
    row = cur.fetchone()
    conn.close()
...
if ret:
    ...
    print(f'✅ CG18 模拟买入成功: {name}({code}) {ret["shares"]}股@{price:.2f}')
