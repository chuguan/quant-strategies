"""
横盘 V41 + 蜡烛图微调(v2:轻量扣分)
"""
NAME = "横盘策略 V41_蜡烛图v2"
MARKET = "flat"

PARAMS = {
    "use_p": 1, "p_w": 0.5, "use_cl": 1, "cl_w": 0.1, "use_vr": 1,
    "use_macd": 1, "macd_w": 1, "dif_bonus": 20, "dif_thresh": 1.5,
    "use_a5": 1, "a5_b": 8, "use_wr": 0, "wr_lo": 15, "wr_lo_b": 8,
    "use_kdj": 1, "j_golden_b": 8, "use_kdj_g": 1, "kdj_g_b": 5,
    ...
    "bonus_kdj_strong": 4, "bonus_flat_combo": 5,
...
def score(stock):
    ...
    cl = stock.get('cl', 50); sl5 = stock.get('slope5', 0); dif = stock.get('dif', 0)
    bp = stock.get('body_pct', 0) or 0; cu = stock.get('cons_up', 0)
    
    # 蜡烛图微调 v2 (通用)
    def candle_adj(c, bp, cu):
        a = 0
        if bp > 6: a -= 1
        if 0 < bp < 1: a -= 1
        if cu >= 5: a -= 2
        if c > 85 and bp < 1: a -= 3
        if c < 30 and bp > 3: a += 2
        return a
    
    if cl < 88 and sl5 > 10 and dif > 2:
        pass
    elif cl < 88:
        p_ = stock.get('p', 0); dif_ = dif; mg_ = stock.get('mg', 0)
        wrv_ = stock.get('wrv', 50); hsl_ = stock.get('hsl', 0); a5_ = stock.get('a5', 0)
        bs = p_ * 2.0 + cl * 0.05
        ms = 0
        ...
        if dif_ > 0.5: bs += 3
        if wrv_ > 50: bs -= 5
        if hsl_ > 15: bs -= 8  # 高换手(>15%)扣分
        t4s = stock.get('t4_shadow', 0)
        if sl5 > 5 and t4s > 20: bs -= 5
        if p_ > 5: bs -= 3
        if stock.get('pos_in_day',50) > 70: bs -= 8
        if cl > 85 and wrv_ < 15: bs -= 8
        bs += candle_adj(cl, bp, cu)
        ms5 = stock.get('ma5_slope', 0) or 0
        if ms5 > 5: bs += 10
        if stock.get("vr",1) < 0.9: bs -= 8
        if dif_ > 2.0 and p_ < 3.0: bs -= 12
        if stock.get("d1",0) > 8: bs -= 8
        return round(bs, 1)
    ...
    # V49：横盘高位超买扣分
    if h>15: bs-=8  # 高换手(>15%)=出货风险
    if h>10: bs-=8  # 横盘中高换手=筹码发散
    if cl > 85 and w < 15: bs -= 8
    ms5 = stock.get('ma5_slope', 0) or 0
    if ms5 > 5: bs += 10
    t4s=stock.get('t4_shadow',0)
    if sl5 > 5 and t4s > 20: bs -= 5
    if pp > 5: bs -= 3; 
    if po > 70: bs -= 8
    if c > 85 and w < 15 and d < 0: bs -= 12
    if v < 0.9: bs -= 8
    if d > 2.0 and pp < 3.0: bs -= 12
    if stock.get("d1",0) > 8: bs -= 8
    return round(bs,1)
