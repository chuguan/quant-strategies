...
V42_DIR = os.path.join(SCRIPTS_DIR, 'strategies', 'V42')
CACHE_DIR = os.path.expanduser('~/AppData/Local/hermes/hermes-agent/cache')
sys.path.insert(0, os.path.join(SCRIPTS_DIR, 'lib')); sys.path.insert(0, V42_DIR); sys.path.insert(0, SCRIPTS_DIR)
...
def backtest_30d():
    ...
    if os.path.exists(PKL):
        try:
            ...
            for dt in data:
                ...
                big_map = {s['code']: (s.get('n', 0) or 0) for s in big_d}
                fixed = 0
                for s in data[dt]:
                    code = s['code']
                    if code in big_map and big_map[code] != 0:
                        if (s.get('n', 0) or 0) != big_map[code]:
                            s['n'] = big_map[code]
                            fixed += 1
                if fixed:
                    print(f'  🔧 {dt}: 刷新{fixed}条n值', flush=True)
        except:
            pass
    ...
    def run_single_backtest(use_250=False):
        ...
        def v10_score(s,code,dt,mk_cn):
            ...
            hi=s.get('high',0) or 0; lo=s.get('low',0) or 0; cl=s.get('close',0) or 0
            stock['amplitude']=round((hi-lo)/max(cl,0.01)*100,1) if hi and lo and cl else 0
...
# ===== 生成HTML =====
def build_html(top10_result, bt_result, prod_result, today_str, data_date_str, buy_date_str, mk_cn, level, ver_id='V50', bt_50=None, bt_100=None, market_avg_pct=0):
    """V10同款淡色模板"""
    ...
    # 动态计算最强/弱行情
    mkt_names_cn_map = {'real_up':'真实涨日','fake_up':'虚涨日','down':'跌日','flat':'横盘'}
    sorted_mkts = sorted([(mk_s[k][0]*100/mk_s[k][1], mk_s[k][0], mk_s[k][1], k) 
                          for k in mkt_names_cn_map if mk_s.get(k) and mk_s[k][1]>0], reverse=True)
    strongest_mkts = [f'{mkt_names_cn_map.get(k,"?")}{r:.0f}%🏆' for r,w,t,k in sorted_mkts[:2]] if sorted_mkts else ['暂无']
    weakest_mkts = [f'{mkt_names_cn_map.get(k,"?")}{r:.0f}%' for r,w,t,k in sorted_mkts[-1:]] if sorted_mkts else ['暂无']
    strongest_str = ' | '.join(strongest_mkts)
    weakest_str = ' | '.join(weakest_mkts)
    ...
    # 冠亚季军胜率（排除待确认天）
    valid_results = [r for r in top3_results if not (r['nh'] == 0 and r['dt'] >= last_cache_date)]
    champ_w = sum(1 for r in valid_results if r['nh']>=2.5)
    second_w = sum(1 for r in valid_results if r['s_nh']>=2.5)
    third_w = sum(1 for r in valid_results if r['t_nh']>=2.5)
    top3_any = sum(1 for r in valid_results if r['nh']>=2.5 or r['s_nh']>=2.5 or r['t_nh']>=2.5)
    n = len(valid_results)
...
<title>V42每日选股报告 {today_str}</title>
<style>
...
<div style="font-size:22px;font-weight:bold;color:#b8860b;text-align:center;margin:10px 0;background:linear-gradient(135deg,#fff8e1,#ffecb3);padding:12px;border-radius:8px;border-bottom:3px solid #b8860b">
  V42 分而治之 · 每日选股报告 (近{p250_ta}天{p250_wi*100//p250_ta}%版 ｜ 14:50真实买点)
</div>
...
<tr><td>50天胜率</td><td>{v50d} (回测)</td></tr>
<tr><td>100天胜率</td><td>{v100d} (回测)</td></tr>
<tr><td>核心武器</td><td>横盘CL<88混合评分 + 冲顶惩罚 + HSL>18%一票否决</td></tr>
...
<div class="footer">
V42 分而治之 | 30天 {wi}/{ta} = {wi*100//ta}% | 实战累计中
...
if __name__ == '__main__':
    ...
    print(f'🚀 V42 每日选股报告 | {today_str}', flush=True)
    ...
    if top10_raw:
        ...
        try:
            ...
            save_realtime_to_datacache(today_str, stocks, indicators, 'tencent:1450-v42')
        except Exception as e:
    ...
    if top10 and bt:
        html = build_html(top10[0], bt, None, today_str, f'{data_label}({data_date})', buy_date, top10[1], top10[2])
        
        # 存档
        arch = os.path.join(SCRIPTS_DIR, 'email_archive')
        os.makedirs(arch, exist_ok=True)
        fp = os.path.join(arch, f'{today_str}_V42_报告.html')
        ...
        try:
            ...
            subj = f'V42每日选股 {today_str} - {top10[1]} - 冠军{top10[0][0][1]["name"]}[{top10[0][0][1]["price"]:.2f}]'
        ...
        # 写入数据库日志
        try:
            log_selection_to_db('V42', today_str, top10[1], top10[2], len(top10[0]), top10[0])
        except Exception as e:
        ...
        print(f'🕐 2:50回测: {p_wi}/{p_ta} = {p_wi*100/p_ta:.1f}%')
    else:
        print('❌ 选股失败')
