#!/usr/bin/env python3
"""V42 模拟盘每日买入 — 13:10执行，从DB读取最新冠军买入"""
...
try:
    ...
    cur = conn.execute(
        ...
        "WHERE version='V42' AND rank=1 ORDER BY run_time DESC LIMIT 1",
    )
    row = cur.fetchone()
    conn.close()
