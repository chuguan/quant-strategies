...
# 实时数据缺技术指标时从data_cache补充
if any('cl' not in s for s in stocks.values()):
    print('[补] data_cache补充CL/WR/DIF...')
    _db = sqlite3.connect(os.path.join(SRC, 'v13_quant.db'), timeout=5)
    _c = _db.execute("SELECT DISTINCT date FROM data_cache WHERE close>0 AND date<=? ORDER BY date DESC LIMIT 1", (TODAY,))
    _r = _c.fetchone()
    if _r:
        _c2 = _db.execute("SELECT code, cl, wr_val, dif_val FROM data_cache WHERE date=? AND close>0", (_r[0],))
        for _r2 in _c2.fetchall():
            if _r2[0] in stocks:
                stocks[_r2[0]]['cl'] = _r2[1]
                stocks[_r2[0]]['wr_val'] = _r2[2]
                stocks[_r2[0]]['dif_val'] = _r2[3]
    _db.close()
...
for ln in LO:
    ...
    for code,s in stocks.items():
        ...
        if s['vol_ratio']<lv['vr_min']or s['vol_ratio']>lv['vr_max']: continue
        ind=inds.get(code)
        if not ind: continue
        if ind['cl']<lv.get('cl_min',0)or ind['cl']>lv.get('cl_max',100): continue
        cand.append((code,s,ind))
    if len(cand)>=10: pool=cand;ul=ln;break
scored=[]
for code,s,ind in pool:
    st={'p':s['p'],'cl':ind['cl'],'vr':s['vol_ratio'],'dif':ind['dif'],'mg':ind['macd_golden'],'wrv':ind['wr'],'jv':ind['j_val'],'kv':ind['k_val'],'dv':ind['d_val'],'a5':1,'kdj_g':ind['kdj_golden'],'pos_in_day':50,'nm':s['name'],'hsl':s.get('hsl',0),'ma5_slope':0,'t4_shadow':0,'slope5':0,'cons_up':0,'d1':0,'d2':0,'d3':0,'close':s.get('close',0) or s.get('price',0) or 0}
    sc=mod.score(st)
    scored.append((sc,[],s,ind,code))
scored.sort(key=lambda x:-x[0])
top10=scored[:10]
print(f'[1] 选股:10只 #1={top10[0][2]["name"]}({top10[0][4]})')
...
subj = f'V51+四维v2 当天选股 {TODAY} {mk_cn}'
