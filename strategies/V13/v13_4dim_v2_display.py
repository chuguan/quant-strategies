#!/usr/bin/env python3
"""V13+四维v2展示版 — 读取V13选股结果，四维v2模板展示"""
...
_cur = _db.execute("SELECT date, market_type, used_level FROM selection_candidates WHERE version='V13' AND date<=? ORDER BY date DESC, run_time DESC LIMIT 1", (TODAY,))
_row = _cur.fetchone()
if not _row:
    print('❌ 没有V13选股记录')
    sys.exit(0)
v50_date, mk_cn, ul = _row
print(f'[V13] 行情: {mk_cn} / {ul}级 ({v50_date})')
# 读最新一次运行的TOP10
_latest = _db.execute("SELECT MAX(run_time) FROM selection_candidates WHERE version='V13' AND date=?", (v50_date,)).fetchone()[0]
_cur = _db.execute("SELECT rank, code, name, price, pct, score, cl, vr, hsl, wr, dif FROM selection_candidates WHERE version='V13' AND date=? AND run_time=? ORDER BY rank LIMIT 10", (v50_date, _latest))
top10_raw = []
...
_db.close()
print(f'[1] V13 TOP10: #1={top10_raw[0]["name"]}({top10_raw[0]["code"]})')
...
subj = f'V13+四维v2 选股展示 {TODAY} {mk_cn}'
