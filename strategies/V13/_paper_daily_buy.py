#!/usr/bin/env python3
"""V13 模拟盘每日买入 — 13:10执行，从DB读取最新冠军买入"""
import sys, os, json, sqlite3
from datetime import datetime
DIR = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, DIR)
from _paper_core import add_position, get_account_summary, load_account, force_sell_all
...
try:
    ...
    cur = conn.execute(
        ...
        "WHERE version='V13' AND rank=1 ORDER BY run_time DESC LIMIT 1",
    )
    row = cur.fetchone()
    conn.close()
...
if not row:
    print(f'⚠️ V13 {today} 无选股记录，跳过买入')
    sys.exit(0)

code, name, price, market_type = row
if not price or price <= 0:
    print(f'⚠️ V13 冠军{name}({code}) 价格无效: {price}')
    sys.exit(0)
...
if ret:
    ...
    print(f'✅ V13 模拟买入成功: {name}({code}) {ret["shares"]}股@{price:.2f}')
