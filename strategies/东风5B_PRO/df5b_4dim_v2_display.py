#!/usr/bin/env python3
"""东风5B PRO+四维v2展示版 — 读取东风5B PRO选股结果，四维v2模板展示"""
...
_cur = _db.execute("SELECT date, market_type, used_level FROM selection_candidates WHERE version='东风5B_PRO' AND date<=? ORDER BY date DESC, run_time DESC LIMIT 1", (TODAY,))
_row = _cur.fetchone()
if not _row:
    print('❌ 没有东风5B_PRO选股记录')
    sys.exit(0)
v50_date, mk_cn, ul = _row
print(f'[东风5B_PRO] 行情: {mk_cn} / {ul}级 ({v50_date})')

# 读最新一次运行的TOP10
_latest = _db.execute("SELECT MAX(run_time) FROM selection_candidates WHERE version='东风5B_PRO' AND date=?", (v50_date,)).fetchone()[0]
_cur = _db.execute("SELECT rank, code, name, price, pct, score, cl, vr, hsl, wr, dif FROM selection_candidates WHERE version='东风5B_PRO' AND date=? AND run_time=? ORDER BY rank LIMIT 10", (v50_date, _latest))
top10_raw = []
...
subj = f'东风5B_PRO+四维v2 选股展示 {TODAY} {mk_cn}'
