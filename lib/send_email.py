...
def send_email(to=None, subject='', body='', html=False, env=None, config='A', force=False):
    """发送邮件
    to: 收件人列表，None则从数据库读
    subject: 主题
    body: 正文
    ...
    # 深夜禁发规则（除非force=True）
    # 0:00~6:00 仅发 1254628314@qq.com，其他收件人不发
    now = datetime.now()
    if not force and night_start <= now.hour < night_end:
        filtered = [r for r in recipients if '1254628314' in r]
        if not filtered:
            print(f'⏰ 凌晨{now.hour}点，跳过邮件发送: {subject}')
            return False
        recipients = filtered
        print(f'⏰ 凌晨{now.hour}点，仅发1254628314: {subject}')
...
def _main():
    """CLI入口"""
    if len(sys.argv) < 2:
        print('用法: python send_email.py "主题" "正文内容" [--html] [--to "收件人"] [--config B]')
        sys.exit(1)
    ...
    send_email(to=to, subject=subject, body=body, html=html, env=env, config=config, force=force)
