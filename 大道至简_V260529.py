...
SUB_STRATEGIES = {
    'real_up': {'module': '大道至简_真实涨日_评分策略', 'name': '真实涨日', 'score_fn': '真实涨日_评分'},
    'fake_up': {'module': '大道至简_虚涨日_评分策略', 'name': '虚涨日', 'score_fn': '虚涨日_评分'},
    'down': {'module': '大道至简_跌日_评分策略', 'name': '跌日', 'score_fn': '跌日_评分'},
    'flat': {'module': '大道至简_横盘_评分策略', 'name': '横盘', 'score_fn': '横盘_评分'},
}

def get_sub_strategy(mkt_key):
    """加载子策略模块获取LEVELS和SCORE权重"""
    import importlib
    info = SUB_STRATEGIES[mkt_key]
    mod = importlib.import_module(info['module'])
    levels = mod.LEVELS
    score_weights = mod.SCORE.copy()
    # 将L0重命名为L，并添加L5
    renamed = []
    for lv in levels:
        name = lv['name']
    ...
    if levels:
        ...
        renamed.append(l5)
    return renamed, info['name'], score_weights
...
def calc_historical_win_rate(mkt_key, module, levels, max_days=333):
    ...
    # 从子策略模块获取评分函数名
    fn_name = ''
    for info in SUB_STRATEGIES.values():
        if info['module'] == module.__name__:
            fn_name = info['score_fn']
            break
    if not fn_name:
        print(f'  ⚠️ 找不到评分函数')
        return None
...
<div style="font-size:22px;font-weight:800;letter-spacing:5px;color:#d4a84b;text-shadow:0 0 20px rgba(212,168,75,0.3)">大道至简 · V260529</div>
...
大道至简 V260529 · L→L5分级 · 8只门槛 · 4行情×4独立评分<br>
...
archive_path = os.path.join(ARCHIVE_DIR, f'{today}_大道至简_V260529.html')
...
send_email(['1254628314@qq.com'],
           f'大道至简 V260529 · 尾盘选股 {today}', html, html=True)
