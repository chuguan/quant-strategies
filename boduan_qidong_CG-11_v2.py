...
【跨年表现】2025: 75.2%  2026: 80.0%  平均77.7%
"""
...
# ═══ CG-11 v2（涨×3+上影+VR+MA5版）评分 ═══
def calc_score(close, pct, vol, v5, pos20, j_val, high, op, atr_pct, body_pct, shadow_pct, ma5_sl, vr):
    sc = max(0, pct) * 3
    sc += (20 if 1 <= vr <= 1.5 else 10 if 1.5 < vr <= 2.5 else 0)
    sc += (20 if shadow_pct < 10 else 10 if shadow_pct < 20 else 0)
    sc += min(ma5_sl * 1.5, 25)
    return sc
...
if __name__ == "__main__":
    ...
    print(f"📅 CG-11 v2（涨×3+上影+VR+MA5版） 选股 — {today}")
    print("📡 加载数据...")
    all_data = load_data()
    print(f"🔍 扫描{today}...")
