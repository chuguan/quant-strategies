#!/usr/bin/env python3
"""
🐷 CG-08 v15（涨上影MA5ATR版）
═══════════════════════════════
尾盘选股 → 次日冲2.5%+卖出

【底仓条件】同CG-07（M1+阳线+站MA5）
【评分】v15：(涨-2%)×4 + 上影<12+20/<20+10 + MA5×0.8(≤16) + ATR×2.5 + VR1~2+10

【跨年表现】2025: 78.9%  2026: 84.4%  平均81.8%
"""
...
def calc_score(close, pct, vol, v5, pos20, j_val, high, op, atr_pct, body_pct, shadow_pct, ma5_sl, vr):
    """v15：(涨-2%)×4 + 上影<12+20/<20+10 + MA5×0.8(≤16) + ATR×2.5 + VR1~2+10"""
    sc = max(0, pct - 2) * 4
    sc += (20 if shadow_pct < 12 else 10 if shadow_pct < 20 else 0)
    sc += min(ma5_sl * 0.8, 16)
    sc += atr_pct * 2.5
    sc += (10 if 1 <= vr <= 2 else 0)
    return sc

# ═══ 以下同CG-07（load_data、pick_today用同一硬过滤+此评分）═══
