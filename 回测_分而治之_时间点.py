"""分而治之V10 时间点回测 — 2:30 vs 2:40 vs 2:50 vs 3:00"""
import sqlite3, os, sys, importlib
from datetime import datetime

TIME_LABEL = sys.argv[1] if len(sys.argv) > 1 else '14:50'
N = int(sys.argv[2]) if len(sys.argv) > 2 else 30
tl_clean = TIME_LABEL.replace(':', '')
VER = f'分而治之V10_{tl_clean}_{N}d'

SCRIPTS_DIR = os.path.expanduser('~/AppData/Local/hermes/scripts')
DB_PATH = os.path.join(SCRIPTS_DIR, 'v13_quant.db')
STRAT_DIR = os.path.join(SCRIPTS_DIR, 'V10')
sys.path.insert(0, STRAT_DIR)

STRATS = {}
for n in ['真实涨日','虚涨日','跌日','横盘']:
    mod = importlib.import_module(f'分而治之_V10_{n}_评分策略')
    STRATS[n] = mod
...
def estimate_p_at_time(s):
    close_p = s.get('p', 0) or 0
    code = s['code']
    prev_close = prev_closes.get(code, 0)
    close = s.get('close', 0) or 0
    if prev_close <= 0 or close <= 0: return close_p
...
def estimate_vr_at_time(s):
    return round((s.get('vr', 1) or 1) * progress, 2)
...
def score_stock(s, code, dt, mk_cn):
    mod = STRATS[mk_cn]
    stock = {
        'p': s.get('_p_est', 0), 'cl': s.get('cl', 50),
        'vr': s.get('_vr_est', 1),
...
print(f'⏰ {TIME_LABEL} 分而治之V10 {N}天回测')
print(f'='*55)

for dt in reversed(dates_n):
    ss = data.get(dt, [])
    for s in ss:
        s['_p_est'] = estimate_p_at_time(s)
