...
def compute_7day_penalty(code, dt, p_today):
    ...
    if feats:
        ...
        d1f = feats.get('d1', 0) or 0
        d2f = feats.get('d2', 0) or 0
        d3f = feats.get('d3', 0) or 0
        cu = feats.get('cons_up', 0) or 0
        pk = feats.get('peak_decay', 0) or 0

        # A: 连续涨停衰竭 — 前5天≥2次涨停，今天涨不过昨天
        zt_cnt = sum(1 for x in [d1f, d2f, d3f, feats.get('d4', 0) or 0, feats.get('d5', 0) or 0] if x >= 9.5)
        if zt_cnt >= 2 and p_ <= d1f: penalty -= 8
        # 加强版：1次涨停+连续涨3天以上+今天涨幅降
        if cu >= 3 and d3f >= 9.5 and p_ < d1f: penalty -= 6

        # B: 高斜率+涨幅递减 — 精准打击赛道股追高
        if sl5 > 10 and d1f > d2f > 0 and p_ <= d1f: penalty -= 6

        # C: 涨停开板后弱反弹
        has_zt_prev = any(x >= 9.5 for x in [d2f, d3f, feats.get('d4',0), feats.get('d5',0)])
        if has_zt_prev and d1f <= 0 and p_ > d1f: penalty -= 8

        # D: 峰值衰减加速
        if pk > 2 and sl5 > 5: penalty -= 5
...
# 跟踪7次失败
fails_track = {'科林电气':0,'大中矿业':0,'融捷股份':0,'通鼎互联':0,'法狮龙':0,'通宇通讯':0,'方正电机':0}
...
for dt in reversed(dates_n):
    ...
    # 跟踪7次失败是否被修正
    if cname in fails_track and win == '✅':
        fails_track[cname] = 1  # 被修好了
    
    results.append((dt, mk_cn, cname, round(champ.get('p', 0) or 0, 1), round(scored[0][0], 1), round(nh, 1), win, mk))

# 报告7次失败修正情况
fixed = [k for k,v in fails_track.items() if v==1]
still_fail = [k for k,v in fails_track.items() if v==0]

# 入库
now_str = datetime.now().strftime('%Y-%m-%d')
for r in results:
    dt, mk_cn, nm, p, score, nh, win, _ = r
    nh_val = nh if win != '⏳' else None
    try:
...
print(f'\n修正情况: 原7次失败中{len(fixed)}次被修正 {fixed}')
...
old = c.fetchone()
if old and old[0]:
    print(f'对比原版: {old[1]}/{old[0]}={old[1]*100/old[0]:.1f}% → 优化版 {wi}/{confirmed_ta}={total_pct:.1f}%')
conn.close()
