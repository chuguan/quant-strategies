...
def verify_strategy_hash(version=None):
    ...
    for fname, db_hash, content in c.fetchall():
        # 在磁盘上找这个文件
        disk_path = os.path.join(SCRIPTS_DIR, 'strategies', version, '评分策略', f'分而治之_V10_{fname.split("_")[0]}_评分策略.py')
        # 模糊查找
        found = False
        for root, dirs, files in os.walk(os.path.join(SCRIPTS_DIR, 'strategies')):
            for f in files:
        ...
        if found and os.path.exists(disk_path):
            ...
            recovery_path = os.path.join(SCRIPTS_DIR, 'strategies', version, '评分策略', f'分而治之_V10_{fname.split("_")[0]}_评分策略.py')
