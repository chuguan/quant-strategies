...
for mw in [0.3, 0.5, 0.7, 1.0]:
    ...
    for dt in target:
        ...
        for s in data.get(dt, []):
            ...
            nh=get_nxt(code,dt)
            cand.append((cl, nm, code, p, vr, hsl, sz, buy, nh, dif, macd_g, above5))
        if not cand: continue
        # 排名 = 涨x2 + CLx1 + 价x0.5 + MACDx权重 + 站上5日线+10
        cand.sort(key=lambda c: (-(c[3]*2 + c[0] + ps(c[7])*0.5 + ms(c[9], c[10])*mw + 10*c[11]), -c[3]))
        ndays+=1
        if cand[0][8]>=2.5: wins+=1
    print(f'MACDx{mw} + 5日+10: {wins}/{ndays}({wins*100/ndays:.1f}%)')
