...
for j_add in [0, 1]:
    ...
    for dt in dates:
        ...
        for s,li in candidates:
            ...
            # J值加分：弱市（flat/down）给J高加分
            if j_add and mkt!='up':
                j_bonus = (jv-60)*0.2 if jv>60 else 0  # J>60每点多0.2分
            else:
                j_bonus = 0
            
            if mkt=='up':
                score=p*3.0+cl*0.1+ps2*0.3+macd_s*0.3+3*above5+hsl_b+duotou_b+BONUS[li]+j_bonus
            elif mkt=='down':
    ...
    name='有J值加分' if j_add else '无J加分'
    print(f"\n=== {name} ===", flush=True)
    for m in sorted(by_month.keys()):
        w,t=by_month[m]
        if m[:4]=='2025' and m[5:7] in ['06','08','10']:
            print(f"  {m}: {w}/{t}({w*100/t:.1f}%) ← 坑月", flush=True)
        else:
            print(f"  {m}: {w}/{t}({w*100/t:.1f}%)", flush=True)
    print(f"  总计: {wins}/{total}({wins*100/total:.1f}%)", flush=True)
