...
dates_30 = backtest_dates[-30:]
# 多取前6天用于7天衰减计算
extended = all_dates[all_dates.index(dates_30[0])-6:all_dates.index(dates_30[-1])+1] if dates_30[0] in all_dates else dates_30
recent = extended
print(f'回测: {dates_30[0]}~{dates_30[-1]} ({len(dates_30)}天)')
...
for dt in dates_30:
    ...
    has_next = (dt != dates_30[-1])  # 非最新日都有D+1
    if has_next:
        nh_display = f'{nh:+.1f}%'
        win = '✅' if nh >= 2.5 else '❌'
        if win == '✅': wi += 1
...
print(f'{"="*55}')
print(f'30天: {wi}/{ta-confirmed_pending} = {wi*100/(ta-confirmed_pending):.1f}% (已确认) | 待验证: {confirmed_pending}天')
...
for r in reversed(results):
    ...
    try:
        ...
        c2.execute('''INSERT OR REPLACE INTO backtest_results
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?)''',
            (now_str, 'V13', dt, mk_cn, nm, nm, p, score, nh_val, win))
        conn2.commit()
        conn2.close()
    except: pass
print(f'💾 已写入 backtest_results 表 (strategy_version=V13)')
