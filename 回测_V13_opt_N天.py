...
def is_momentum_exhausted(s, code, dt):
    ...
    # ─── 新增优化规则 ───
    # 规则A: 连续涨停衰竭 — 前5天≥2次涨停，今天涨不动了
    zt_count = sum(1 for x in [d1, d2, d3, d4] if x >= 9.5)
    if zt_count >= 1 and cu >= 3 and pv < d1:
        return True

    # 规则B: 高斜率追高衰竭 — sl5>10且涨幅递减(d1>d2)，追高没力
    if sl5 > 10 and d1 > d2 and d1 > 0 and pv <= d1:
        return True

    # 规则C: 涨停开板后反弹假冲 — 有涨停，sl5为负(回落中)，今天反弹但弱
    recent_p = [d1, d2, d3, d4]
    has_zt = any(x >= 9.5 for x in recent_p)
    if has_zt and sl5 < 0 and pv > d1:
        return True

    # 规则D: 峰值衰减加强 — 原来只检查pk>5，再加更敏感的
    if pk > 1.5 and sl5 > 5:
        return True
...
def compute_7day_penalty(code, dt, p_today):
    ...
    zt_cnt = sum(1 for x in d_vals if x >= 9.5)
    if zt_cnt >= 2 and p_ <= d1: penalty -= 12
    elif zt_cnt >= 1 and p_ <= d1 and (d1 < d2 or d1 < d3): penalty -= 8

    # 规则B惩罚: 高斜率衰竭 — sl5来自特征
    feats = features.get((code, dt), {})
    if feats:
        sl5 = feats.get('slope5', 0) or 0
        if sl5 > 10 and d1 > d2: penalty -= 8
        if sl5 > 15 and p_ <= d1: penalty -= 10

    # 规则C惩罚: 涨停开板后反弹 — 有涨停，回落再拉但弱
    has_zt_prev = any(x >= 9.5 for x in [d2, d3, d4, d5])
    if has_zt_prev and d1 <= 0 and p_ > d1: penalty -= 10
...
old = c.fetchone()
if old and old[0]:
    print(f'原版 {old[1]}/{old[0]} = {old[1]*100/old[0]:.1f}% → 优化版 {wi}/{confirmed_ta} = {total_pct:.1f}%')
else:
    print(f'优化版 {wi}/{confirmed_ta} = {total_pct:.1f}%')
